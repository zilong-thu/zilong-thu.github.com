...
(function() {
  ...
  _.delay = function(func, wait) {
    //** args：把本函数的 `func`  `wait`两个变量去掉后的其他参数
    var args = slice.call(arguments, 2);
  ...
  _.keys = function(obj) {
    ...
    if (nativeKeys){
      return nativeKeys(obj);  //** 最开始的时候定义了 nativeKeys = Object.keys;
    }
    var keys = [];
    for (var key in obj) {
      if (_.has(obj, key)) {
  ...
  //** 用于判断一个给定的 数组、字符串长丢是否为0，或者对象是否没有可枚举的自有属性
  ...
  //** DOM元素的节点类型（nodeType）都是1（严格的数字1，而非字符串“1”）
  ...
  //** 判断一个对象是否是一个数组
  //** 优先使用ECMA5的原生判断方法 Array.isArray()
  ...
  //** 根据 arguments 类数组的一个独有的属性：callee（指向arguments所属的函数自身），来判断是否是arguments。
